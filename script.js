...
function showProductDetail() {
  ...
  if (vitrine && detail) {
    ...
    // Atualiza o Swiper de detalhes após ele ficar visível
    if (window.detailSwiper) {
      setTimeout(() => {
        window.detailSwiper.update();
      }, 100);
    }
  }
}
...
function hideProductDetail() {
  ...
  if (vitrine && detail) {
    ...
    // Atualiza o Swiper da vitrine após ele ficar visível
    if (window.vitrineSwiper) {
      setTimeout(() => {
        window.vitrineSwiper.update();
      }, 100);
    }
  }
}
...
function applyCoupon() {
  ...
  if (input === "MILGRAU15") {
    ...
    if (priceDisplay) {
      priceDisplay.innerHTML = `TOTAL: <strike>R$ 99,90</strike> <span style='color:var(--vinho)'>R$ ${(CONSTANTS.PRODUTO_PRECO - state.desconto).toFixed(2).replace('.', ',')}</span>`;
    }
    showSuccess("Cupom MILGRAU15 aplicado! Você ganhou R$ 15,00 de desconto.", "CUPOM APLICADO!");
  } else if (input !== "") {
    if (priceDisplay) priceDisplay.innerHTML = "TOTAL: R$ 99,90";
